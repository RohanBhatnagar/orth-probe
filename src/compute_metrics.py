...
from metric_functions import compute_energy, compute_complexity, compute_innovation, compute_expansion
        ...
        for sample_key in tqdm(sample_keys, desc="Processing samples"):
            ...
            for dset_name in grp.keys():
                if dset_name.startswith("layer_"):
                    ...
                    layers_data[layer_idx][residual_type] = torch.from_numpy(
                        grp[dset_name][:]
                    ).to(torch.float32).cuda()
            
            # Process each layer
            for layer_idx in sorted(layers_data.keys()):
                if "pre" not in layers_data[layer_idx] or "post" not in layers_data[layer_idx]:
                    continue
                ...
                for t in range(seq_len):
                    ...
                    if t >= cfg.history_window:
                        delta_window = delta_r[t-cfg.history_window:t+1]  # [window_size, d_model]
...
@hydra.main(version_base=None, config_path=CONFIG_DIR, config_name="compute_metrics_config")
def _hydra_main(cfg):
    init_cfg(cfg)
    set_seed(cfg.seed)
    ...
    with h5py.File(residuals_path, "r") as f:
        for i, (sample_key, layer_dict) in enumerate(list(results.items())[:3]):
            ...
            for layer_idx in sorted(layer_dict.keys())[:3]:
                ...
                layer_complexity = np.mean([m["complexity"] for m in metrics_list if m["complexity"] is not None])
                layer_innovation = np.mean([m["innovation"] for m in metrics_list if m["innovation"] is not None])
                layer_expansion = np.mean([m["expansion"] for m in metrics_list if m["expansion"] is not None])
